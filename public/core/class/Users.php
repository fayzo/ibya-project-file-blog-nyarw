...
class Users{
     ...
     public function login($email,$password,$datetime)
    {
       ...
       $sql1= $mysqli->query("SELECT user_id ,username,profile_img FROM users WHERE username ='{$email}' or email ='{$email}'");
     ...
     public function update($table,$fields=array(),$user_id)
    {
        ...
        $sql="SELECT user_id FROM $table WHERE user_id='$user_id' AND {$column}";
        $query= $mysqli->query($sql);
        $row = $query->fetch_assoc();
        return  $row['user_id'];
        // var_dump($sql);
     ...
     public function forgotUsername($table,$arrayselects=array(),$conditions = array())
     {
        ...
        $query_no_username= $mysqli->query($sql1);
        $row_no_username = $query_no_username->fetch_assoc();
        // var_dump($sql1);

            return $row_no_username['user_id'];
    }
